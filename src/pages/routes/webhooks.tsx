            ...
            <div class="flex flex-wrap gap-3">
              {WEBHOOK_EVENTS.map((event) => (
                <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                  <input type="checkbox" name="events" value={event} class="rounded border-gray-300 dark:border-gray-600" />
                  {event}
                </label>
              ))}
            </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-800">
                <th class="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">URL</th>
                <th class="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Events</th>
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td class="px-4 py-3">
                    <div class="flex flex-wrap gap-1">
                      {(Array.isArray(webhook.events) ? webhook.events : []).map((ev: string) => (
                  ...
                  <td class="px-4 py-3">
                    <form method="POST" action={`/dashboard/webhooks/${webhook.id}/delete`}>
                      <button
                        type="submit"
