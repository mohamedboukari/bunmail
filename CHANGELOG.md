...
- **Schema:** adds the `smtp_submission_usage` table (#123) — per-`(api_key, UTC day)` `accepted`/`rejected` counters, unique on `(api_key_id, day)`, FK → `api_keys` `ON DELETE CASCADE`. Migration `drizzle/0009_parched_marvel_boy.sql`. Run `bun run db:migrate` (or rebuild via `docker compose up -d --build`, which migrates on boot) before deploying.
