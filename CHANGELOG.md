...
- **Dependency refresh (`ncu -u`) + removed the `overrides` block.** Bumped runtime deps `mailparser` 3.9.10→3.9.15, `nodemailer` 9.0.1→9.0.5, `smtp-server` 3.19.0→3.19.3, `fast-xml-parser` 5.9.2→5.10.1, plus dev tooling (`eslint`, `typescript-eslint`, `prettier`, `knip`, `lint-staged`, `@commitlint/*`, `bumpp`, `globals`) — all minor/patch, no majors. Full suite + lint + type-check green. **All `package.json` `overrides` were removed** — every one was a point-in-time CVE floor (nodemailer #115, linkify-it #139, and yaml/file-type/brace-expansion/picomatch/defu/flatted #56), and with the refreshed tree they're all redundant: natural resolution now meets or exceeds every former floor (`nodemailer` 9.0.5, `linkify-it` 5.0.2 via `mailparser`/`smtp-server`; `picomatch` even resolves *higher* at 4.0.5), verified by re-resolving with no overrides. This also clears the `EOVERRIDE` conflict `npm`/`ncu` reported. The Trivy fs + image scans (every PR/build) remain the backstop and will re-flag any future transitive regression.

- **Build provenance attestation for the published image (#145).** The Docker workflow now signs **SLSA build provenance** for every image it pushes to GHCR, keyless via Sigstore (`actions/attest-build-provenance`, OIDC — no signing key to store or rotate; the chosen path over standalone cosign for a GHCR-only consumer). The attestation is bound to the immutable image **digest** (not the tag) and pushed to the registry, so anyone can verify the image was built by this repo's CI with `gh attestation verify oci://ghcr.io/<owner>/bunmail@<digest> --owner <owner>`. Also corrected the `trivy-action` pin comment to its release tag (`v0.36.0`; the SHA was already that release's commit).
- **CI hardening — least-privilege, cancellation, timeouts (#145).** Locked the GitHub Actions setup down without changing any build behaviour: added a top-level `permissions: contents: read` block to `ci.yml` (it inherited the repo-default token scope before); added `concurrency` with `cancel-in-progress` to `ci.yml`, `security.yml`, and `codeql.yml` so a follow-up push cancels its own superseded run (deliberately **not** on `release.yml`/`docker.yml` — never abort a tag publish mid-push); added `timeout-minutes` job backstops across all workflows; and set `persist-credentials: false` on the checkouts that don't push (CI, CodeQL, Trivy fs/image, Docker). Actions were already fully SHA-pinned.
...
- **SMTP submission: per-key daily quotas + usage stats endpoint (#123).** The submission server (#120) gains an optional per-API-key daily send quota via `SMTP_SUBMISSION_DAILY_QUOTA` (default `0` = unlimited) — counted per UTC day; over-quota submissions get SMTP `452` (temporary; resets at 00:00 UTC) and don't consume quota. A new read-only REST endpoint `GET /api/v1/smtp-submission/stats?days=N` (Bearer-auth, scoped to the calling key) returns per-day accepted/rejected counts, window totals, and quota status (`daily`/`usedToday`/`remaining`; `daily`/`remaining` are `null` when unlimited). Backed by a new `smtp_submission_usage` table (one row per key per UTC day). Applies to the SMTP submission path only — the REST send API is unaffected. Cross-key/instance-wide analytics remain a follow-up. See [docs/smtp-submission.md](docs/smtp-submission.md#per-key-daily-quotas-123).
- **SMTP submission server — send *through* BunMail over SMTP (#120).** A new AUTH-required SMTP listener (default port `587`) lets any SMTP-capable app (Infisical, Netbird, Dify, a NestJS/Nodemailer backend, …) relay mail through BunMail with **no code changes** — point the app's SMTP settings at BunMail and use a `bm_live_…` API key as the password (any username, e.g. `apikey`; mirrors SendGrid). Authenticated messages are parsed and handed to the existing outbound pipeline (`createEmail` → queue → DKIM → direct-to-MX), so they get the same retries, suppression checks, DKIM signing, and webhooks as a REST send. BCC is preserved (envelope recipients absent from the visible `To`/`Cc` headers are delivered blind). Distinct from the inbound receiver (which has AUTH disabled and validates recipient domains) — here the open-relay guard is authentication, so authenticated clients can send to any recipient. Off by default: set `SMTP_SUBMISSION_ENABLED=true` and uncomment the submission port line in `docker-compose.yml`. STARTTLS is advertised when `SMTP_SUBMISSION_TLS_CERT`/`_KEY` are set; otherwise plaintext AUTH is allowed for trusted-network (same-host / private Docker) setups. A per-IP failed-AUTH throttle blunts API-key brute-forcing. New env: `SMTP_SUBMISSION_ENABLED`, `SMTP_SUBMISSION_PORT`, `SMTP_SUBMISSION_TLS_CERT`, `SMTP_SUBMISSION_TLS_KEY`, `SMTP_SUBMISSION_RATE_LIMIT_*`, `SMTP_SUBMISSION_AUTH_RATE_LIMIT_*`. No schema changes (reuses the `emails` table). **v1 limitation:** attachments and arbitrary custom headers are not relayed (the outbound pipeline has no field for them yet) — tracked as a follow-up. See [docs/smtp-submission.md](docs/smtp-submission.md).

### Changed

- **Schema:** adds the `smtp_submission_usage` table (#123, migration `0009`) and the `api_keys.allowed_senders` column (#126, migration `0010`, `jsonb NOT NULL DEFAULT '[]'` — existing keys default to unrestricted). Run `bun run db:migrate` (or rebuild via `docker compose up -d --build`, which migrates on boot) before deploying.
...
- **Dashboard login brute-force protection (#109).** `POST /dashboard/login` now rate-limits failed password attempts per client IP — **5 failures / 15 minutes** by default, then HTTP `429` with a `Retry-After` header until the window clears. A successful login resets the counter. The client IP is resolved with a spoof-resistant, count-from-the-right read of `X-Forwarded-For`: new env `DASHBOARD_TRUSTED_PROXY_HOPS` (default `0` = trust only the raw socket IP; set to `1` behind a single reverse proxy such as nginx/Caddy/Cloudflare). Tunable via `DASHBOARD_LOGIN_RATE_LIMIT_ENABLED` (default `true`), `DASHBOARD_LOGIN_RATE_LIMIT_MAX` (default `5`), and `DASHBOARD_LOGIN_RATE_LIMIT_WINDOW` (seconds, default `900`). State is in-memory and per-replica (same caveat as the API rate limiter). No schema or API changes.
